...
- **Text representation:** DTMT's readable `.strings` output is SJSON, not JSON. Quoting and escapes must be decoded before comparing displayed text. The helpers do this; `null` denotes a missing language, while `""` is a present empty translation.
